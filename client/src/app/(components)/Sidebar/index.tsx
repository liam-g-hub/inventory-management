...
const SidebarLink = ({
    ...
    return (
        <Link href = {href}>
            ...
            >
                <Icon className = "w-6 h-6 !test-gray-700" />
...
const Sidebar = () => {
    ...
    const sidebarClassNames = `fixed flex flex-col ${isSidebarCollapsed ? "w-0 md:w-16" : "w-72 md:w-64"} bg-white h-full shadow-md transition-all duration-300 overflow-hidden h-full shadow-md z-40`;

    return (
        <div className = {sidebarClassNames}>
            {/* Top Logo */}
            <div className = {`flex gap-3 justify-between md:justify-around items-center pt-8 ${isSidebarCollapsed ? "px-5" : "px-8"}`}>
                <div>logo</div>
                <h1 className = {`${isSidebarCollapsed ? "hidden" : "block"} font-extrabold text-2xl`}>Inventory Management System</h1>
            ...
            <div className = {`${isSidebarCollapsed ? "hidden" : "block"} mb-10`}>
                <p className = "text-center text-gray-500">&copy; 2024 DeMaf Fabrications Inc.</p>
            </div>
